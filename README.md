...
- 同步內容分成**作業**（作業、作業到期提醒、即時更新）與**課表**（所有課程、課程顏色、自訂課程名稱）兩組，**逐項**選擇要不要同步
- **跨平台**：macOS、iPad、iPhone 與 Android 只要開啟 TigerSync 就會互相同步，作業提醒與即時更新的設定也一起帶過去
- F-Droid 版沒有 Google 推播服務，TigerSync 只保留「取得必要資訊」
