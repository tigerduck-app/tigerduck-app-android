...
class MailNotificationPlannerTest {
    ...
    @Test
    fun `up to five mails notify one by one, oldest first, with clean text`() {
        val plan = MailNotificationPlanner.plan(
            listOf(m(3), m(2, from = null, subject = "  "), m(4, from = MailAddress(null, "a@x.tw"), subject = "x\u202ey")),
            noSender = "（沒有寄件者）", noSubject = "（沒有主旨）",
        ) as MailNotificationPlanner.Plan.Individual
        assertEquals(listOf(2L, 3L, 4L), plan.items.map { it.uid })
        assertEquals("（沒有寄件者）", plan.items[0].title)
        assertEquals("（沒有主旨）", plan.items[0].text)
        assertEquals("教務處", plan.items[1].title)
        assertEquals("a@x.tw", plan.items[2].title)
        assertEquals("xy", plan.items[2].text)
    }

    @Test
    fun `more than five collapse into one summary`() {
        assertEquals(MailNotificationPlanner.Plan.Summary(6), MailNotificationPlanner.plan((1L..6L).map { m(it) }, "", ""))
        assertNull(MailNotificationPlanner.plan(emptyList(), "", ""))
    }
