...
fun TigerPullToRefresh(
    ...
    //
    // The three lambdas are built here rather than inside the class, so the class itself holds
    // nothing from the composition: latestIsRefreshing and latestOnRefresh are still read through
    // their remembered State on every call, exactly as they were when this was an anonymous
    // object, and the threshold haptic keeps the Context here where it belongs.
    val connection = remember(appBar, searchReveal) {
        PullChromeConnection(
            ...
            onThresholdHaptic = { Haptics.perform(context, HapticScenario.PullToRefresh) },
        )
    }
...
/**
 * The nested-scroll connection behind [TigerPullToRefresh]: the refresh pull, the hiding app bar
 * and the search drawer, sharing one gesture between them.
 *
 * A named class rather than an anonymous object inside the composable, so that every rule in here
 * -- the finger-down gate that is the whole of the refresh-hang fix, the source gate, and the
 * order the three consumers unwind in -- can be exercised from a plain JVM unit test. None of it
 * needs a Context, a frame clock or a composition: [Animatable] is pure Kotlin, and the one thing
 * that did need a Context (the threshold haptic) arrives as [onThresholdHaptic].
 *
 * Everything the composable used to capture is passed in instead, and [TigerPullToRefresh] still
 * builds exactly one of these per (appBar, searchReveal) pair, in the same `remember`.
 *
 * @param isRefreshing read on every call rather than captured once: the caller's own
 *   `isRefreshing` changes while a pull is still in flight.
 * @param onThresholdHaptic fired once each time a pull crosses [thresholdPx] on the way up.
 */
internal class PullChromeConnection(
    ...
    private val appBar: AppBarState?,
    private val searchReveal: SearchRevealState?,
    ...
    override suspend fun onPreFling(available: Velocity): Velocity {
        ...
        if (dragY.value > 0f) {
            val triggered = dragY.value >= thresholdPx && !isRefreshing()
