...
class MailSender(
    ...
    /**
     * Sends [mail], then files the copy over [sessions] -- the caller's already-authenticated
     * connection, not a second one opened here.
     */
    suspend fun send(
        credentials: MailCredentials,
        mail: OutgoingMail,
        sentFolder: String?,
        sessions: SentCopySession,
    ): SendResult {
        ...
        val copy = if (sentFolder == null) SentCopy.NotAttempted else fileCopy(built, sentFolder, sessions)
